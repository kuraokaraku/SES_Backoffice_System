...
| Excel操作 | openpyxl |
| PDF解析 | pdfplumber |
| AI連携 | OpenAI API gpt-4o-mini（勤務表パース補助・契約書フィールド抽出） |
| 認証 | Django標準認証 |
| データベース（本番） | MySQL |
| デプロイ先 | AWS EC2 (43.207.97.181) / ドメイン: itfl-backoffice.com |
| CI/CD | GitHub Actions（テスト → EC2自動デプロイ） |
...
**契約書AI自動入力**:
フォーム上の「契約書からAI自動入力」ボタンから Excel/PDF をアップロードすると、
OpenAI API（gpt-4o-mini）がフィールドを抽出し、スプリットビューで確認しながらフォームへ反映できる。

- 左パネル: Excelプレビュー（抽出根拠セルをハイライト表示）
- 右パネル: AI抽出結果（編集可能）＋信頼度スコア
- 「フォームに反映」ボタンで登録フォームへ一括コピー
...
| /ap/ | ap_list | ap_list |
| /ap/payables/\<id\>/payments/ | ap_payment_create | ap_payment_create |
| /ap/payments/\<id\>/delete/ | ap_payment_delete | ap_payment_delete |
| /api/contract-parse/ | contract_parse_upload | contract_parse_upload |
...
| 0006 | SalesDeal, SalesProject, SalesStatusChange, SalesAction 追加（後に削除） |
| 0007 | Payable, PayableLine, PayablePayment 追加 |
| 0008 | Assignment.order_period を ServiceContract へコピー |
| 0009 | Assignment.order_period_start_ym / end_ym を削除 |
| 0010 | Sales系モデルを全削除 |
...
| openai | AI補助（勤務表パース強化・契約書フィールド抽出） |
...
テスト用の一時DBを自動作成・実行後に削除する（本番DBに影響なし）。

### 11.2 テスト構成（63本）

| ファイル | 対象 | 本数 |
|---------|------|------|
| test_invoice_calculator.py | 切り捨て計算・超過/控除/交通費・支払期日 | 22 |
| test_invoice_db.py | 採番（generate_invoice_number）・合計再計算（recalculate_totals） | 17 |
| test_contracts.py | get_active_contract の契約期間判定 | 7 |
| test_xlsx_extractor.py | Excelパーサ（セル抽出・HTML描画・列フィルタ） | 11 |
| test_views.py | 主要ページの疎通・未ログイン時リダイレクト | 13 |

---

## 12. CI/CD

### 12.1 GitHub Actions（`.github/workflows/deploy.yml`）
...
push to main
    ↓
[test ジョブ] テスト全63本を実行（SQLite/DEBUG=True）
    ↓ 通過した場合のみ
[deploy ジョブ] AWS EC2 へ SSH 接続
    → git pull → pip install → migrate → collectstatic → gunicorn 再起動
```

テストが1本でも失敗するとデプロイはスキップされる。

---

## 13. 既知の制約・今後の改善候補

1. **勤務表パーサ**: 汎用パーサのため、特殊フォーマットでは信頼度が低い場合がある
2. **入金管理**: 物理削除のみ（論理削除・監査ログ未実装）
3. **レガシーモデル**: Freelancer, BusinessPartner等はAssignment系に移行済みだが、旧URLは残存
4. **メディアファイル**: `re_path` で直接配信（本番ではNginx推奨）
5. **タイムゾーン**: settings.py で UTC のまま（Asia/Tokyo が望ましい）
